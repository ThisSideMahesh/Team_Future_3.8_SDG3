...
export function HealthcareProviderView() {
  ...
  const healthRecordsQuery = useMemoFirebase(() => searchedPatientId ? query(collection(firestore, `records`), where('patient_id', '==', searchedPatientId)) : null, [firestore, searchedPatientId]);
  ...
  const criticalAlerts = healthRecords?.flatMap(record => record.allergies.map(allergy => ({
    id: `${record.record_id}-allergy-${allergy}`,
    title: 'Allergy Alert',
    details: allergy,
    type: 'Allergy'
  }))) || [];
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Search for a patient who has granted you consent using their Patient ID.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col sm:flex-row items-start gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="w-full sm:w-auto flex-grow">
                    ...
                    <FormControl>
                      <Input placeholder="Enter a Patient ID with consent (e.g., PAT_001)" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <div className="w-full sm:w-auto pt-0 sm:pt-8 flex items-center gap-2">
                ...
                <Button type="button" variant="outline" onClick={() => { /* Implement QR Scanner */ }} className="w-full sm:w-auto">
      ...
      { (patientError || (searchedPatientId && !isLoading && !patient)) && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Could not retrieve patient record. This is likely because the Patient ID is incorrect or the patient has not granted consent for you to view their record.
          </AlertDescription>
        </Alert>
      )}
      ...
      {patient && !isLoading && (
        <div className="space-y-8">
            <Card>
                <CardHeader className="flex flex-row items-center gap-4 space-y-0">
                    ...
                    <div>
                        ...
                        <CardDescription>ID: {patient.patient_id} &bull; DoB: {new Date(patient.dob).toLocaleDateString()}</CardDescription>
                    </div>
                </CardHeader>
            ...
            <div>
              ...
              <p className="text-muted-foreground mb-4">This timeline is a unified view of the patient's records from all participating institutions.</p>
